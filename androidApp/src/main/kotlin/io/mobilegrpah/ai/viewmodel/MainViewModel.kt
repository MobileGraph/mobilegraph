...
package io.mobilegrpah.ai.viewmodel
...
import io.mobilegrpah.ai.ApplicationLogger
import io.mobilegrpah.ai.BuildConfig
        ...
        sessionMap.values.forEach { session ->
            viewModelScope.launch {
                session.events.collect { event ->
                    val eventStr =
                        when (event) {
                            is MobileGraphEvent.RequestStarted -> "Session ${session.sessionId.takeLast(4)}: Started"
                            is MobileGraphEvent.RequestCompleted -> "Session ${session.sessionId.takeLast(4)}: Success"
                            is MobileGraphEvent.RequestFailed -> "Session ${session.sessionId.takeLast(4)}: Failed - ${event.errorMessage}"
                        }
